...
export async function getUpcomingEvents(): Promise<{ events: CalendarEvent[], logs: string[], viviendas: any[] }> {
    const supabase = getSupabaseClient(); // ✅ cliente creado en runtime
    ...
    // Trigger background sync to Alquileres table
    try {
        await syncIcalToAlquileres(icalEvents, viviendas || [], log, supabase);
    } catch (e: any) {
        log(`Error in auto-sync: ${e.message}`);
    }
...
async function syncIcalToAlquileres(icalEvents: CalendarEvent[], viviendas: any[], log: (msg: string) => void, supabase: any) {
    if (icalEvents.length === 0) return;
